...
- [ ] **Task 1: Create Role-Based Authorization Policies** (AC: 1, 2, 3, 5)

  - [ ] Analyze current roles from Story 1.3: Dispatcher, Customer, Contractor
  - [ ] Create authorization policies in `Program.cs` using `AddAuthorization()`:
    - [ ] Policy: `"DispatcherOnly"` → requires role `"Dispatcher"`
    - [ ] Policy: `"CustomerOnly"` → requires role `"Customer"`
    - [ ] Policy: `"ContractorOnly"` → requires role `"Contractor"`
    - [ ] Optional: Combined policy `"AnyAuthenticatedUser"` → requires any authenticated user
  - [ ] Verify `[Authorize]` middleware is configured (from Story 1.3)
  - [ ] Test: Verify 403 is returned when authenticated user lacks required role

- [ ] **Task 2: Add RBAC to AuthController Endpoints** (AC: 1, 4, 5)

  - [ ] Apply `[AllowAnonymous]` to:
    - [ ] `POST /api/v1/auth/login` (public, no auth needed)
  - [ ] Keep endpoints public (no additional [Authorize] needed since they're already public)
  - [ ] Note: Logout and Refresh endpoints require valid JWT (already handled by JWT middleware)
...
- [ ] **Task 3: Create Role-Filtered Data Access Helper Methods** (AC: 6, 7)

  - [ ] Create `SmartScheduler.Application/Services/IAuthorizationService.cs`:
    - [ ] Interface methods:
      - [ ] `FilterDataByRole(userId: Guid, role: string, data: IQueryable<T>): IQueryable<T>` (generic)
      - [ ] `GetCurrentUserIdFromContext(claims: ClaimsPrincipal): Guid` - extract user ID from JWT claims
      - [ ] `ValidateUserOwnsResource(userId: Guid, resourceOwnerId: Guid): bool` - verify user owns resource
      - [ ] `ValidateUserRole(requiredRole: string, userRole: string): bool` - explicit role validation
  - [ ] Create `SmartScheduler.Application/Services/AuthorizationService.cs` implementing above:
    - [ ] `FilterDataByRole()` logic (role-specific filtering is intentional - each role has different business rules):
      - [ ] If role = "Dispatcher": Return all data (dispatchers see everything - admin/orchestrator view)
      - [ ] If role = "Customer": Filter to only resources owned by customer (jobs, assignments, reviews) - privacy boundary
      - [ ] If role = "Contractor": Filter to only contractor's own data (assigned jobs, reviews about this contractor) - isolation to prevent competitor visibility
    - [ ] Error handling: Return empty collection if filter yields no results (not 403 - privacy: don't reveal if resource exists)
  - [ ] Register in DI container: `services.AddScoped<IAuthorizationService, AuthorizationService>()`

- [ ] **Task 4: Create Example Endpoints with Role-Based Protection** (AC: 1, 2, 3, 4, 5, 6)

  - [ ] Create `SmartScheduler.API/Controllers/ContractorsController.cs`:

    - [ ] Route: `[Route("api/v1/contractors")]`
    - [ ] `GET /api/v1/contractors` - List all contractors
      - [ ] `[Authorize]` - requires authentication
      - [ ] Returns all active contractors (paginated)
      - [ ] No role restriction: All authenticated users can view contractor list
      - [ ] Response: `{ id, name, location, tradeType, avgRating, reviewCount }`
    - [ ] `POST /api/v1/contractors` - Create contractor
      - [ ] `[Authorize(Roles = "Dispatcher")]` - only dispatchers can create
      - [ ] Request: `{ name, location, tradeType, workingHours, phone }`
      - [ ] Returns: Created contractor with 201 Created
      - [ ] Unauthorized (no role): 403 Forbidden with message "Forbidden"

  - [ ] Create `SmartScheduler.API/Controllers/JobsController.cs`:

    - [ ] Route: `[Route("api/v1/jobs")]`
    - [ ] `GET /api/v1/jobs` - List jobs (role-filtered)
      - [ ] `[Authorize]` - requires authentication
      - [ ] Logic:
        - [ ] If role = "Dispatcher": Return all jobs (pending, assigned, completed)
        - [ ] If role = "Customer": Return only jobs created by this customer (use `AuthorizationService.FilterDataByRole()`)
        - [ ] If role = "Contractor": Return only jobs assigned to this contractor (use `AuthorizationService.FilterDataByRole()`)
      - [ ] Implementation: `var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;` extract from JWT
      - [ ] Call: `var filteredJobs = await _authorizationService.FilterDataByRole(userId, User.FindFirst(ClaimTypes.Role)?.Value, jobsQuery);`
      - [ ] Returns paginated list with role-appropriate data
    - [ ] `POST /api/v1/jobs` - Create job
      - [ ] `[Authorize(Roles = "Customer")]` - only customers can submit jobs
      - [ ] Request: `{ jobType, location, desiredDateTime, description }`
      - [ ] Auto-populate `CustomerId` from JWT claims (not from request)
      - [ ] Returns: Created job with 201 Created
    - [ ] `GET /api/v1/jobs/{id}` - Get single job
      - [ ] `[Authorize]` - requires authentication
      - [ ] Logic:
        - [ ] Fetch job from database
        - [ ] Verify user is authorized to view: `_authorizationService.ValidateUserOwnsResource(userId, job.CustomerId)` if Customer
        - [ ] Or verify: `_authorizationService.ValidateUserOwnsResource(userId, assignment.ContractorId)` if assigned Contractor
        - [ ] Or allow all if Dispatcher
        - [ ] If not authorized: Return 403 Forbidden with message "You do not have permission to view this job"
      - [ ] Returns job details with 200 OK, or 403 if not authorized

  - [ ] Create `SmartScheduler.API/Controllers/RecommendationsController.cs`:
    - [ ] Route: `[Route("api/v1/recommendations")]`
    - [ ] `GET /api/v1/recommendations?jobType=...&location=...` - Get contractor recommendations
      - [ ] `[Authorize(Roles = "Dispatcher")]` - only dispatchers can request recommendations
      - [ ] Unauthorized user (Customer/Contractor): 403 Forbidden
      - [ ] Returns top 5 recommended contractors ranked by score
      - [ ] Response: `{ contractors: [ { id, name, score, rating, distance, travelTime } ] }`
...
- [ ] **Task 6: Create Unit Tests for RBAC** (AC: 4, 5, 8)

  - [ ] Create `SmartScheduler.API.Tests/Controllers/ContractorsControllerTests.cs`:

    - [ ] Test POST /contractors:
      - [ ] Dispatcher can create contractor: 201 Created
      - [ ] Customer token → 403 Forbidden "Forbidden"
      - [ ] Contractor token → 403 Forbidden "Forbidden"
      - [ ] No token (unauthenticated) → 401 Unauthorized
    - [ ] Test GET /contractors:
      - [ ] Authenticated (any role) → 200 OK with contractor list
      - [ ] Unauthenticated → 401 Unauthorized

  - [ ] Create `SmartScheduler.API.Tests/Controllers/JobsControllerTests.cs`:

    - [ ] Test GET /jobs (role-filtered):
      - [ ] Dispatcher token → 200 OK with all jobs (>0 jobs if seeded)
      - [ ] Customer token → 200 OK with only their jobs (filtered)
      - [ ] Contractor token → 200 OK with only their assigned jobs (filtered)
      - [ ] Unauthenticated → 401 Unauthorized
    - [ ] Test POST /jobs:
      - [ ] Customer token → 201 Created with CustomerId auto-populated
      - [ ] Dispatcher token → 403 Forbidden
      - [ ] Contractor token → 403 Forbidden
      - [ ] Unauthenticated → 401 Unauthorized
    - [ ] Test GET /jobs/{id} (authorization):
      - [ ] Job owner (Customer) → 200 OK
      - [ ] Assigned Contractor → 200 OK
      - [ ] Dispatcher → 200 OK
      - [ ] Unrelated Contractor → 403 Forbidden "You do not have permission to view this job"
      - [ ] Unrelated Customer → 403 Forbidden "You do not have permission to view this job"

  - [ ] Create `SmartScheduler.API.Tests/Controllers/RecommendationsControllerTests.cs`:

    - [ ] Test GET /recommendations:
      - [ ] Dispatcher token → 200 OK with top 5 contractors
      - [ ] Customer token → 403 Forbidden
      - [ ] Contractor token → 403 Forbidden
      - [ ] Unauthenticated → 401 Unauthorized

  - [ ] Create `SmartScheduler.Application.Tests/Services/AuthorizationServiceTests.cs`:
    - [ ] Test FilterDataByRole:
      - [ ] Dispatcher role → Returns all jobs
      - [ ] Customer role → Returns only their jobs (filtered by CustomerId)
      - [ ] Contractor role → Returns only their assigned jobs (filtered by ContractorId)
      - [ ] Empty result for no matching data (not exception)
    - [ ] Test ValidateUserOwnsResource:
      - [ ] User owns resource (IDs match) → true
      - [ ] User doesn't own resource → false
      - [ ] Null inputs handled gracefully
    - [ ] Test GetCurrentUserIdFromContext:
      - [ ] Valid claims → Returns correct UserId
      - [ ] Missing NameIdentifier claim → Throws exception or returns Guid.Empty
...
- [ ] **Task 8: Update Exception Handling Middleware** (AC: 4, 5)

  - [ ] Update `SmartScheduler.API/Middleware/ExceptionHandlingMiddleware.cs`:
    - [ ] Catch `UnauthorizedAccessException` → Return 403 Forbidden:
      ```json
      {
        "error": {
          "code": "FORBIDDEN",
      ...
      ```
    - [ ] Already handles `SecurityTokenException` (from Story 1.3) → 401 Unauthorized
